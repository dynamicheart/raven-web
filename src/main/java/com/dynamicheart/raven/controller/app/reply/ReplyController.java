...
import com.dynamicheart.raven.controller.app.reply.populator.CreateReplyFormPopulator;
...
public class ReplyController {
    ...
    @Inject
    private CreateReplyFormPopulator createReplyFormPopulator;
    ...
    //add post reply api
    @RequestMapping(value = "/api/v1/ravens/{ravenId}/replies",method = RequestMethod.POST)
    @Authorization
    @ApiResponses({
            @ApiResponse(code = 200, response = ReplyInfoFields.class,  message = "create a reply")
    })
    public ResponseEntity<?> get(@PathVariable String ravenId, @RequestBody CreateReplyForm createReplyForm, @CurrentUser @ApiIgnore User currentUser) throws Exception{
        Reply reply=createReplyFormPopulator.populate(createReplyForm);

        if(!reply.getRavenId().equals(ravenId))
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body(new GenericResponseBody(Message.MESSAGE_FORBIDDEN));
        ...
        if(!raven.getAddresseeIds().contains(currentUser.getId()) || !reply.getUserId().equals(currentUser.getId())){
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body(new GenericResponseBody(Message.MESSAGE_FORBIDDEN));
        }
